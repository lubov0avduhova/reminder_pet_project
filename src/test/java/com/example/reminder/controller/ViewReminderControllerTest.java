...
@WebMvcTest(controllers = ViewReminderController.class)
@AutoConfigureMockMvc(addFilters = false)
public class ViewReminderControllerTest {
    ...
    @Test
    public void ListController_GetPaginatedList_ReturnsTotalAndCurrentList() throws Exception {
        // arrange
        ...
        actions.andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.total").value(responses.size()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.current", hasSize(2)))
                .andExpect(MockMvcResultMatchers.jsonPath("$.current[0].title", CoreMatchers.is("Помыть машину")))
                .andExpect(MockMvcResultMatchers.jsonPath("$.current[0].description", CoreMatchers.is("Найти свободное время")));
